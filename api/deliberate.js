export default async function handler(req, res) {
  ...
  if (!apiKey) {
    return res.status(501).json({ error: "API Key missing in Vercel settings" });
  }
  ...
  try {
    ...
    // THE FIX: We use v1beta and gemini-1.5-flash. 
    // This is the specific "address" Google wants.
    ...
    if (!resp.ok) {
      // If Flash fails, try Pro on the same v1beta path
