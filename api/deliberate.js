...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ---- Follow-up mode: a short, blended answer from the board ----
    if (followup) {
      const priorText = priorResult?.responses
        ? priorResult.responses.map(r => `${r.name}: ${r.text}`).join("\n")
        : "(none)";
      const fPrompt = `You are "Council". The user already heard from their board on a decision and now asks a FOLLOW-UP question. Answer in 3-5 sentences as a blended voice of the board, weaving in 2-3 named members' authentic perspectives where it helps. Be specific and warm. Respectful with sacred figures; no fabricated exact quotes.
...
Return ONLY JSON: { "followupAnswer": "..." }`;
      ...
      const fParsed = JSON.parse(fContent);
      return res.status(200).json(fParsed);
    }
